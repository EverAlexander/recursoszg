                ...
                <v-card-text>
                    <v-container>
                        <v-row dense class="mt-3">
                            ...
                            <!-- Descripción -->
                            <v-col cols="12" sm="12" md="6">
                                <base-input label="Descripción" v-model="v$.editedItem.descripcion.$model"
                                    :rules="v$.editedItem.descripcion" outlined dense />
                            </v-col>

                            <!-- ID de Marca oculto -->
                            <input type="hidden" v-model="v$.editedItem.id_marca.$model" />
                        </v-row>
...
export default {
    ...
    data() {
        return {
            ...
            dialogDelete: false,
            headers: [
                { title: "Nombre de la marca", key: "nombre" },
    ...
    methods: {
        ...
        async getDataFromApi(options) {
            ...
            this.debounce = setTimeout(async () => {
                try {
                    const { data } = await brandsApi.get("/select", {
                        params: {
